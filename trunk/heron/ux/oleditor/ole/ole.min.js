...
"file","file",null,b),c=document.createElement("select");c.setAttribute("name","format_select");c.setAttribute("id","format_select");for(var d,e=0;e<this.formats.length;++e){d=document.createElement("option");var f=this.formats[e].formatter;"string"==typeof f&&(f=eval("new "+f+"()"));this.formatters[this.formats[e].name]=f;d.setAttribute("value",this.formats[e].name);d.innerHTML=this.formats[e].name;d.fileExt=this.formats[e].fileExt;d.mimeType=this.formats[e].mimeType;c.appendChild(d)}b.appendChild(c);
for(var h in this.params)b=this.createInputElm(null,h,"hidden",this.params[h],b);a.appendChild(b);this.showUploadDialog(a);var k=this;document.getElementById("oleFile").onchange=function(a){if(a=k.fileInputValue(this))if(a=a.split("."),1<a.length){a="."+a[a.length-1].toLowerCase();for(var b=document.getElementById("format_select"),c=b.options,d=0;d<c.length;++d)if(a==c[d].fileExt){b.selectedIndex=d;break}}}},showMessage:function(a,b){this.getDialog().show({title:OpenLayers.i18n("oleUploadFeature"),
...
"width: 0; height: 0; border: none;");b.src="about:blank";return b},removeIFrame:function(){var a=document.getElementById("upload_iframe");a&&a.parentNode.removeChild(a);this.iframe=null},cancel:function(){this.deactivate()},parseFeatures:function(a){var b=document.getElementById("format_select"),b=this.formatters[b.options[b.selectedIndex].value];if(!b)return null;this.fileProjection&&(b.internalProjection=this.layer.map.baseLayer.projection,b.externalProjection=this.fileProjection);a=b.read(a);
if(!a||1>a.length)return null;var c;a.constructor!=Array&&(a=[a]);for(b=0;b<a.length;++b)c?c.extend(a[b].geometry.getBounds()):c=a[b].geometry.getBounds();this.bounds=c;return a},uploadFeature:function(){var a=document.getElementById("oleFile");if(this.fileInputValue(a)){this.iframe=this.createIFrame("upload_iframe");document.body.appendChild(this.iframe);var b=this,c=b.iframe,d=function(){c.detachEvent?c.detachEvent("onload",d):c.removeEventListener("load",d,!1);var a;c.contentDocument?a=c.contentDocument.body.innerHTML:
c.contentWindow?a=c.contentWindow.document.body.innerHTML:c.document&&(a=c.document.body.innerHTML);a&&"escape"==b.params.encoding&&(a=a.replace(/&quot;/g,'"').replace(/&gt;/g,">").replace(/&lt;/g,"<").replace(/&amp;/g,"&"));(a=b.parseFeatures(a))?(b.layer.addFeatures(a),b.layer.map.zoomToExtent(b.bounds),setTimeout(function(){b.deactivate()},250)):(b.deactivate(),b.showMessage(OpenLayers.i18n("oleUploadFeatureNone")))};c.addEventListener&&c.addEventListener("load",d,!0);c.attachEvent&&c.attachEvent("onload",
d);document.getElementById("upload_form").submit()}else this.cleanUp(this),this.showMessage(OpenLayers.i18n("oleUploadFeatureNoFile"),OpenLayers.Function.bind(this.delayedOpenDialog,this))},createInputElm:function(a,b,c,d,e){var f=document.createElement("input");f.setAttribute("type",c);f.id=a;f.name=b;f.value=d?d:null;e&&e.appendChild(f);return e},fileInputValue:function(a){return a.files?1==a.files.length?a.files[0].name:null:a.value},CLASS_NAME:"OpenLayers.Editor.Control.UploadFeature"});/*
  2011 geOps
 @license    https://github.com/geops/ole/blob/master/license.txt
 @link       https://github.com/geops/ole
*/
