...
"text"==this.featureTypes[d]&&this.activeControls.push("DrawText");d=0;for(e=this.sourceLayers.length;d<e;d++)c={editor:this,layer:this.sourceLayers[d],controls:["OpenLayers.Editor.Control.ImportFeature"]},this.sourceLayers[d].events.register("featureselected",c,this.selectionChanged),this.sourceLayers[d].events.register("featureunselected",c,this.selectionChanged),this.sourceLayers[d].styleMap=new OpenLayers.StyleMap({"default":new OpenLayers.Style({fillColor:"#0c0",fillOpacity:0.8,strokeColor:"#070",
strokeWidth:2,graphicZIndex:1,pointRadius:5}),select:new OpenLayers.Style({fillColor:"#fc0",strokeColor:"#f70",graphicZIndex:2}),temporary:new OpenLayers.Style({fillColor:"#fc0",fillOpacity:0.8,strokeColor:"#f70",strokeWidth:2,graphicZIndex:2,pointRadius:5})}),this.map.addLayer(this.sourceLayers[d]);this.map.editor=this;this.map.addLayer(this.editLayer);this.map.addControl(new OpenLayers.Editor.Control.LayerSettings(this));this.undoRedoActive&&this.map.addControl(new OpenLayers.Editor.Control.UndoRedo(this.editLayer));
...
CLASS_NAME:"OpenLayers.Editor"});OpenLayers.Editor.Control=OpenLayers.Class(OpenLayers.Control,{initialize:function(a){OpenLayers.Control.prototype.initialize.apply(this,[a])},CLASS_NAME:"OpenLayers.Editor.Control"});OpenLayers.Editor.VERSION_NUMBER="1.0-beta1";/*
  2011 geOps
 @license    https://github.com/geops/ole/blob/master/license.txt
 @link       https://github.com/geops/ole
*/
...
if(!1!==this.layer.events.triggerEvent("sketchcomplete",{feature:b})&&a.getArea()>=this.minArea){var b=a.getVertices(),c,d=0,e=this.layer.features.length;a:for(;d<e;d++){var f=this.layer.features[d];c=!0;for(var h=0,k=b.length;h<k;h++)f.geometry.intersects(b[h])||(c=!1);if(c){f.state=OpenLayers.State.UPDATE;this.layer.events.triggerEvent("beforefeaturemodified",{feature:f});f.geometry.components.push(a.components[0]);this.layer.drawFeature(f);this.layer.events.triggerEvent("featuremodified",{feature:f});
...
null;a=this.feature;if(!a)return!1;var b=a.editControl;if(!b)return!1;var c=document.getElementById("olLabelInput");b.setLabelText(a,c.value);b.removePopup(a);c.value||a.layer.removeFeatures([a])},isTextFeature:function(a){return a?a.featureType&&"text"==a.featureType:!1},onFeatureSelected:function(a){a=a.feature;this.isTextFeature(a)&&a.layer.drawFeature(a,this.selectStyle)},onFeatureUnselected:function(a){a=a.feature;this.isTextFeature(a)&&a.layer.drawFeature(a,this.defaultStyle)},onBeforeFeatureAdded:function(a){var b=
...
var c=document.getElementById("olLabelInput"),d=this;c.onkeypress=function(a){if(13==(window.event?window.event.keyCode:a.keyCode))d.setLabelText(b,c.value),d.removePopup(b)};c.focus();this.popup=a},CLASS_NAME:"OpenLayers.Editor.Control.DrawText"});/*
 ...
 @link       https://github.com/geops/ole
*/
OpenLayers.Editor.Control.UploadFeature=OpenLayers.Class(OpenLayers.Control,{EVENT_TYPES:["featureuploaded"],layer:null,title:OpenLayers.i18n("oleUploadFeature"),url:"",params:{action:"upload",mime:"text/html",encoding:"escape"},formats:[{name:"GeoJSON",fileExt:".json",mimeType:"text/plain",formatter:"OpenLayers.Format.GeoJSON"}],fileProjection:null,formatters:{},initialize:function(a,b){this.layer=a;this.title=OpenLayers.i18n("oleUploadFeature");OpenLayers.Control.Button.prototype.initialize.apply(this,
[b])},activate:function(){var a=OpenLayers.Control.prototype.activate.call(this);a&&this.openDialog();return a},deactivate:function(){var a=OpenLayers.Control.prototype.deactivate.call(this);a&&this.cleanUp(this);return a},cleanUp:function(a){a.map.editor.dialog.hide();a.iframe&&a.removeIFrame()},delayedOpenDialog:function(){var a=this;setTimeout(function(){a.openDialog()},200)},openDialog:function(){var a=document.createElement("div");a.appendChild(document.createElement("p"));var b=document.createElement("form");
b.setAttribute("id","upload_form");b.setAttribute("method","POST");b.setAttribute("action",this.url);b.setAttribute("target","upload_iframe");b.setAttribute("enctype","multipart/form-data");b.setAttribute("encoding","multipart/form-data");var b=this.createInputElm("oleFile","file","file",null,b),c=document.createElement("select");c.setAttribute("name","format_select");c.setAttribute("id","format_select");for(var d,e=0;e<this.formats.length;++e){d=document.createElement("option");var f=this.formats[e].formatter;
"string"==typeof f&&(f=eval("new "+f+"()"));this.formatters[this.formats[e].name]=f;d.setAttribute("value",this.formats[e].name);d.innerHTML=this.formats[e].name;d.fileExt=this.formats[e].fileExt;d.mimeType=this.formats[e].mimeType;c.appendChild(d)}b.appendChild(c);for(var h in this.params)b=this.createInputElm(null,h,"hidden",this.params[h],b);a.appendChild(b);this.showUploadDialog(a);var k=this;document.getElementById("oleFile").onchange=function(a){if(a=k.fileInputValue(this))if(a=a.split("."),
1<a.length){a="."+a[a.length-1].toLowerCase();for(var b=document.getElementById("format_select"),c=b.options,d=0;d<c.length;++d)if(a==c[d].fileExt){b.selectedIndex=d;break}}}},showMessage:function(a,b){this.map.editor.dialog.show({title:OpenLayers.i18n("oleUploadFeature"),content:a,close:b})},showUploadDialog:function(a){this.map.editor.dialog.show({title:OpenLayers.i18n("oleUploadFeature"),content:a,save:OpenLayers.Function.bind(this.uploadFeature,this),cancel:OpenLayers.Function.bind(this.cancel,
this),saveButtonText:"oleDialogOkButton",noHideOnSave:!0})},createIFrame:function(a){var b=document.createElement("iframe");b.setAttribute("id",a);b.setAttribute("name",a);b.setAttribute("width","0");b.setAttribute("height","0");b.setAttribute("border","0");b.setAttribute("style","width: 0; height: 0; border: none;");b.src="about:blank";return b},removeIFrame:function(){var a=document.getElementById("upload_iframe");a&&a.parentNode.removeChild(a);this.iframe=null},cancel:function(){this.deactivate()},
parseFeatures:function(a){var b=document.getElementById("format_select"),b=this.formatters[b.options[b.selectedIndex].value];if(!b)return null;this.fileProjection&&(b.internalProjection=this.layer.map.baseLayer.projection,b.externalProjection=this.fileProjection);a=b.read(a);if(!a||1>a.length)return null;var c;a.constructor!=Array&&(a=[a]);for(b=0;b<a.length;++b)c?c.extend(a[b].geometry.getBounds()):c=a[b].geometry.getBounds();this.bounds=c;return a},uploadFeature:function(){var a=document.getElementById("oleFile");
if(this.fileInputValue(a)){this.iframe=this.createIFrame("upload_iframe");document.body.appendChild(this.iframe);var b=this,c=b.iframe,d=function(){c.detachEvent?c.detachEvent("onload",d):c.removeEventListener("load",d,!1);var a;c.contentDocument?a=c.contentDocument.body.innerHTML:c.contentWindow?a=c.contentWindow.document.body.innerHTML:c.document&&(a=c.document.body.innerHTML);a&&"escape"==b.params.encoding&&(a=a.replace(/&quot;/g,'"').replace(/&gt;/g,">").replace(/&lt;/g,"<").replace(/&amp;/g,
"&"));(a=b.parseFeatures(a))?(b.layer.addFeatures(a),b.layer.map.zoomToExtent(b.bounds),setTimeout(function(){b.deactivate()},250)):(b.deactivate(),b.showMessage(OpenLayers.i18n("oleUploadFeatureNone")))};c.addEventListener&&c.addEventListener("load",d,!0);c.attachEvent&&c.attachEvent("onload",d);document.getElementById("upload_form").submit()}else this.cleanUp(this),this.showMessage(OpenLayers.i18n("oleUploadFeatureNoFile"),OpenLayers.Function.bind(this.delayedOpenDialog,this))},createInputElm:function(a,
b,c,d,e){var f=document.createElement("input");f.setAttribute("type",c);f.id=a;f.name=b;f.value=d?d:null;e&&e.appendChild(f);return e},fileInputValue:function(a){return a.files?1==a.files.length?a.files[0].name:null:a.value},CLASS_NAME:"OpenLayers.Editor.Control.UploadFeature"});/*
  2011 geOps
 @license    https://github.com/geops/ole/blob/master/license.txt
 @link       https://github.com/geops/ole
*/
...
b,c);if(!a._moving){var f=a.dragControl.handlers.drag.evt,h=!(!a._setfeature&&a.preserveAspectRatio)&&!(f&&f.shiftKey),k=new OpenLayers.Geometry.Point(d,e),f=a.center;this.rotate(-a.rotation,f);k.rotate(-a.rotation,f);var m=this.x-f.x,g=this.y-f.y,l=m-(this.x-k.x),n=g-(this.y-k.y);a.irregular&&!a._setfeature&&(m-=(this.x-k.x)/2,g-=(this.y-k.y)/2);this.x=d;this.y=e;k=1;a.feature.geometry instanceof OpenLayers.Geometry.Point?g=1:h?(g=1E-5>Math.abs(n)?1:g/n,k=(1E-5>Math.abs(l)?1:m/l)/g):(l=Math.sqrt(l*
l+n*n),g=Math.sqrt(m*m+g*g)/l);a._moving=!0;a.box.geometry.rotate(-a.rotation,f);delete a._moving;a.box.geometry.resize(g,f,k);a.box.geometry.rotate(a.rotation,f);a.transformFeature({scale:g,ratio:k});a.irregular&&!a._setfeature&&(m=f.clone(),m.x+=1E-5>Math.abs(d-f.x)?0:this.x-d,m.y+=1E-5>Math.abs(e-f.y)?0:this.y-e,a.box.geometry.move(this.x-d,this.y-e),a.transformFeature({center:m}))}},f=function(b,c){var d=this.x,e=this.y;OpenLayers.Geometry.Point.prototype.move.call(this,b,c);if(!a._moving){var f=
a.dragControl.handlers.drag.evt,f=f&&f.shiftKey?45:1,k=a.center,h=this.x-k.x,g=this.y-k.y;this.x=d;this.y=e;d=Math.atan2(g-c,h-b);d=Math.atan2(g,h)-d;d*=180/Math.PI;a._angle=(a._angle+d)%360;d=a.rotation%f;if(Math.abs(a._angle)>=f||0!==d)d=Math.round(a._angle/f)*f-d,a._angle=0,a.box.geometry.rotate(d,k),a.transformFeature({rotation:d})}},h=Array(8),k=Array(4),g,n,p,q="sw s se e ne n nw w".split(" "),l=0;8>l;++l)g=this.box.geometry.components[l],n=new OpenLayers.Feature.Vector(g.clone(),{role:q[l]+
"-resize"},"string"==typeof this.renderIntent?null:this.renderIntent),0==l%2&&(p=new OpenLayers.Feature.Vector(g.clone(),{role:q[l]+"-rotate"},"string"==typeof this.rotationHandleSymbolizer?null:this.rotationHandleSymbolizer),p.geometry.move=f,g._rotationHandle=p,k[l/2]=p),g.move=b,g.resize=c,g.rotate=d,n.geometry.move=e,g._handle=n,h[l]=n;this.rotationHandles=k;this.handles=h}});OpenLayers.Editor.Control.FixedAngleDrawing=OpenLayers.Class(OpenLayers.Control,{CLASS_NAME:"OpenLayers.Editor.Control.FixedAngleDrawing",active:!1,sketchVerticesAmount:null,guides:null,initialize:function(a){this.guides=[];OpenLayers.Control.prototype.initialize.call(this);this.layer=a},activate:function(){var a=OpenLayers.Control.prototype.activate.call(this);if(a)this.layer.events.on({sketchstarted:this.onSketchStarted,sketchmodified:this.onSketchModified,sketchcomplete:this.onSketchComplete,scope:this});
...
f.b,d),f=this.createWorldBoundaryPoint(f.m,d.right,f.b,d));d=a.geometry.components[0];d.x=c.x;d.y=c.y;var h=a.geometry.components[1];h.x=f.x;h.y=f.y;c=OpenLayers.Layer.Vector.prototype.drawFeature.apply(this,arguments);d.x=e.x1;d.y=e.y1;h.x=e.x2;h.y=e.y2}else console.warn("Failed to render a feature",a)}return c},moveTo:function(a,b,c){OpenLayers.Layer.Vector.prototype.moveTo.apply(this,arguments);this.features.forEach(function(a){this.drawFeature(a)},this)}});/*
  2011 geOps
 @license    https://github.com/geops/ole/blob/master/license.txt
 @link       https://github.com/geops/ole
*/
...
oleSnappingSettingsTolerance:"Tolerancia de l'ajust automàtic",oleSplitFeature:"Partir la geometria seleccionada"});OpenLayers.Lang.de=OpenLayers.Util.extend(OpenLayers.Lang.de,{oleCleanFeature:"Selektierte Geometrien bereinigen",oleDeleteFeature:"Selektierte Geometrien löschen",oleDragFeature:"Geometrie verschieben",oleDrawHole:"Loch schneiden",oleDrawPolygon:"Fläche erstellen",oleDrawPath:"Linie erstellen",oleDrawPoint:"Punkt erstellen",oleImportFeature:"Selektierte Geometrien importieren",oleImportFeatureSourceLayer:"Keine Ebenen für den Import gefunden",oleImportFeatureSourceFeature:"Keine selektierten Geometrien für den Import gefunden.",
oleLayerSettingsImportHeader:"Import",oleLayerSettingsImportLabel:"Layer als Quelle für Import verwenden",oleLayerSettingsLegendHeader:"Legende",oleLayerSettingsOpacityHeader:"Opazität in %",oleMergeFeature:"Selektierte Geometrien verschmelzen",oleMergeFeatureSelectFeature:"Bitte mindestens 2 Flächen auswählen.",oleModifyFeature:"Geometrien bearbeiten",oleNavigation:"Navigation",olePixelUnit:"px",oleSelectFeature:"Geometrien selektieren",oleSnappingSettings:"Snapping Einstellungen",oleSnappingSettingsLayer:"Snapping Layer",
oleSnappingSettingsTolerance:"Snapping Toleranz in px",oleSplitFeature:"Selektierte Geometrien teilen",oleTransformFeature:"Geometrie skalieren, drehen und verschieben",oleCADTools:"CAD Funktionen",oleCADToolsDialogParallelDrawing:"Paralleles Zeichnen aktivieren",oleCADToolsDialogGuidedDrawing:"Geführtes Zeichnen aktivieren",oleCADToolsDialogShowLayer:"Hilfslinien anzeigen",oleCADToolsDialogTolerance:"px Toleranz",oleDialogCancelButton:"Abbrechen",oleDialogSaveButton:"Speichern",oleDialogOkButton:"Okay"});OpenLayers.Lang.en=OpenLayers.Util.extend(OpenLayers.Lang.en,{oleCleanFeature:"Clean selected geometry",oleDeleteFeature:"Delete selected geometry",oleDeleteAllFeatures:"Delete all geometries",oleDownloadFeature:"Download geometries",oleDownloadFeatureEmpty:"No geometries to download",oleDownloadFeatureFileFormat:"Select a file format",oleUploadFeature:"Upload geometries from local file",oleUploadFeatureNoFile:"No file specified. Please choose a file and try again.",oleUploadFeatureNone:"No or unreadable geometries found in file",
oleDragFeature:"Drag geometry",oleDrawHole:"Draw hole",oleDrawPolygon:"Draw polygon",oleDrawPath:"Draw path",oleDrawPoint:"Draw point",oleDrawText:"Draw text label",oleDrawTextEdit:"Add label text in box,<br>then press enter or close popup",oleImportFeature:"Import selected geometry",oleImportFeatureSourceLayer:"Found no source layer.",oleImportFeatureSourceFeature:"Please select a geometry first.",oleLayerSettingsImportHeader:"Import",oleLayerSettingsImportLabel:"Use as source layer",oleLayerSettingsLegendHeader:"Legend",
oleLayerSettingsOpacityHeader:"Opacity (%)",oleMergeFeature:"Merge selected geometry",oleMergeFeatureSelectFeature:"Please select at least 2 geometries.",oleModifyFeature:"Modify geometry",oleNavigation:"Navigation",olePixelUnit:"px",oleSelectFeature:"Select geometry",oleSnappingSettings:"Snapping settings",oleSnappingSettingsLayer:"Snapping layer",oleSnappingSettingsTolerance:"Snapping tolerance",oleSplitFeature:"Split selected geometry",oleTransformFeature:"Scale, rotate and move geometry",oleCADTools:"CAD Tools",
oleCADToolsDialogParallelDrawing:"Parallel Drawing",oleCADToolsDialogGuidedDrawing:"Guided Drawing",oleCADToolsDialogShowLayer:"Show Guide Lines",oleCADToolsDialogTolerance:"px tolerance",oleDialogCancelButton:"Cancel",oleDialogSaveButton:"Save",oleDialogOkButton:"Okay"});OpenLayers.Lang.hu=OpenLayers.Util.extend(OpenLayers.Lang.hu,{oleCleanFeature:"Kijelölt geometria javítása",oleDeleteFeature:"Kijelölt geometria törlése",oleDragFeature:"Geomertia mozgatása",oleDrawHole:"Lyuk rajzolása",oleDrawPolygon:"Poligon rajzolása",oleDrawPath:"Vonal rajzolása",oleDrawPoint:"Pont rajzolása",oleImportFeature:"Kijelölt geometriák importálása",oleImportFeatureSourceLayer:"Nem található forrásréteg.",oleImportFeatureSourceFeature:"Kérem, előbb válasszon ki egy geometriát!",oleLayerSettingsImportHeader:"Importálás",
oleLayerSettingsImportLabel:"Használat forrásrétegként",oleLayerSettingsLegendHeader:"Jelmagyarázat",oleLayerSettingsOpacityHeader:"Átlátszóság (%)",oleMergeFeature:"Kijelölt geometriák összevonása",oleMergeFeatureSelectFeature:"Kérem, válasszon ki legalább két geometriát!",oleModifyFeature:"Geometria módosítása",oleNavigation:"Navigáció",olePixelUnit:"px",oleSelectFeature:"Geometria kiválasztása",oleSnappingSettings:"Illesztés beállításai",oleSnappingSettingsLayer:"Illesztő réteg",oleSnappingSettingsTolerance:"Illesztés toleranciája",
oleSplitFeature:"Kijelölt geometria szétvágása",oleTransformFeature:"Geometria átméretezése, forgatása és mozgatása",oleCADTools:"CAD Eszközök",oleCADToolsDialogParallelDrawing:"Párhuzamos rajzolás",oleCADToolsDialogGuidedDrawing:"Segédvonalas rajzolás",oleCADToolsDialogShowLayer:"Segédvonalak megjelenítése",oleCADToolsDialogTolerance:"px tolerancia",oleDialogCancelButton:"Mégse",oleDialogSaveButton:"Mentés",oleDialogOkButton:"Ok"});OpenLayers.Lang.nl=OpenLayers.Util.extend(OpenLayers.Lang.nl,{oleCleanFeature:"Geselecteerde geometrie opschonen",oleDeleteFeature:"Geselecteerde geometrie verwijderen",oleDeleteAllFeatures:"Alle geometrieën verwijderen",oleDownloadFeature:"Download geometrieën",oleDownloadFeatureEmpty:"Geen geometrieën om te downloaden",oleDownloadFeatureFileFormat:"Selecteer een file formaat",oleUploadFeature:"Upload geometrieën vanuit lokaal bestand",oleUploadFeatureNoFile:"Geen bestand geselecteerd. Probeer opnieuw en kies een bestand.",
oleUploadFeatureNone:"Geen of onleesbare geometrieën in bestand",oleDragFeature:"Geometrie verschuiven",oleDrawHole:"Gat knippen in geometrie",oleDrawPolygon:"Teken vlak",oleDrawPath:"Teken lijn",oleDrawPoint:"Teken punt",oleDrawText:"Teken tekst label",oleImportFeature:"Importeer geselecteerde geometrie",oleImportFeatureSourceLayer:"Geen bron-laag.",oleImportFeatureSourceFeature:"Selecteer eerst een geometrie.",oleLayerSettingsImportHeader:"Importeer",oleLayerSettingsImportLabel:"Gebruik als bron-laag",
olelayerSettingsLegendHeader:"Legenda",olelayerSettingsOpacityHeader:"Ondoorzichtigheid (%)",oleMergeFeature:"Combineer geselecteerde geometrie",oleMergeFeatureSelectFeature:"Selecteer tenminste 2 geometrieën.",oleModifyFeature:"Geometrie wijzigen",oleNavigation:"Navigatie",olePixelUnit:"px",oleSelectFeature:"Selecteer geometrie",oleSnappingSettings:"Uitlijn-instellingen",oleSnappingSettingsLayer:"Uitlijnen laag",oleSnappingSettingsTolerance:"Uitlijn-tolerantie",oleSplitFeature:"Splits geselecteerde geometrie",
oleTransformFeature:"Geometrie schalen, roteren en verschuiven",oleCADTools:"CAD Functies",oleCADToolsDialogParallelDrawing:"Parallel tekenen aktiveren",oleCADToolsDialogGuidedDrawing:"Geleid tekenen aktiveren",oleCADToolsDialogShowLayer:"Toon hulplijnen",oleCADToolsDialogTolerance:"px tolerantie",oleDialogCancelButton:"Annuleren",oleDialogSaveButton:"Bewaren",oleDialogOkButton:"Oke"});
