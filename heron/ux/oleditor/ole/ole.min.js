Array.prototype.forEach||(Array.prototype.forEach=function(a,b){var c,d;if(null==this)throw new TypeError("this is null or not defined");var e=Object(this),f=e.length>>>0;if("[object Function]"!=={}.toString.call(a))throw new TypeError(a+" is not a function");b&&(c=b);for(d=0;d<f;){var k;Object.prototype.hasOwnProperty.call(e,d)&&(k=e[d],a.call(c,k,d,e));d++}});/*
  2011 geOps
...
if(!1!==this.layer.events.triggerEvent("sketchcomplete",{feature:b})&&a.getArea()>=this.minArea){var b=a.getVertices(),c,d=0,e=this.layer.features.length;a:for(;d<e;d++){var f=this.layer.features[d];c=!0;for(var k=0,g=b.length;k<g;k++)f.geometry.intersects(b[k])||(c=!1);if(c){f.state=OpenLayers.State.UPDATE;this.layer.events.triggerEvent("beforefeaturemodified",{feature:f});f.geometry.components.push(a.components[0]);this.layer.drawFeature(f);this.layer.events.triggerEvent("featuremodified",{feature:f});
...
"file","file",null,b),c=document.createElement("select");c.setAttribute("name","format_select");c.setAttribute("id","format_select");for(var d,e=0;e<this.formats.length;++e){d=document.createElement("option");var f=this.formats[e],k=f.formatter;"string"==typeof k&&(k=eval("new "+k+"()"));k.fileProjection=f.fileProjection;this.formatters[f.name]=k;d.setAttribute("value",f.name);d.innerHTML=f.name;d.fileExt=f.fileExt;d.mimeType=f.mimeType;c.appendChild(d)}b.appendChild(c);var c=document.createElement("p"),
g=document.createElement("input");g.type="checkbox";g.id="oleUploadFeatureReplace";g.name="uploadFeatureReplace";g.value="true";g.checked=!0;g.defaultChecked=!0;this.setReplaceFeatures(g.checked);c.appendChild(g);OpenLayers.Event.observe(c,"click",OpenLayers.Function.bind(function(a){OpenLayers.Event.stop(a,!0);this.setReplaceFeatures(g.checked)},this));d=document.createElement("label");d.htmlFor="oleUploadFeatureReplace";d.appendChild(document.createTextNode(OpenLayers.i18n("oleUploadFeatureReplace")));
c.appendChild(d);b.appendChild(c);for(var h in this.params)b=this.createInputElm(null,h,"hidden",this.params[h],b);a.appendChild(b);this.showUploadDialog(a);var n=this;document.getElementById("oleFile").onchange=function(a){if(a=n.fileInputValue(this))if(a=a.split("."),1<a.length){a="."+a[a.length-1].toLowerCase();for(var b=document.getElementById("format_select"),c=b.options,d=0;d<c.length;++d)if(a==c[d].fileExt){b.selectedIndex=d;break}}}},showMessage:function(a,b){this.getDialog().show({title:OpenLayers.i18n("oleUploadFeature"),
...
"width: 0; height: 0; border: none;");b.src="about:blank";return b},removeIFrame:function(){var a=document.getElementById("upload_iframe");a&&a.parentNode.removeChild(a);this.iframe=null},cancel:function(){this.deactivate()},parseFeatures:function(a){var b=document.getElementById("format_select"),b=this.formatters[b.options[b.selectedIndex].value];if(!b)return null;this.fileProjection&&(b.internalProjection=this.layer.map.baseLayer.projection,b.externalProjection=b.fileProjection?b.fileProjection:
this.fileProjection);a=b.read(a);if(!a||1>a.length)return null;var c;a.constructor!=Array&&(a=[a]);for(b=0;b<a.length;++b)c?c.extend(a[b].geometry.getBounds()):c=a[b].geometry.getBounds();this.bounds=c;return a},uploadFeature:function(){var a=document.getElementById("oleFile");if(this.fileInputValue(a)){this.iframe=this.createIFrame("upload_iframe");document.body.appendChild(this.iframe);var b=this,c=b.iframe,d=function(){c.detachEvent?c.detachEvent("onload",d):c.removeEventListener("load",d,!1);
var a;c.contentDocument?a=c.contentDocument.body.innerHTML:c.contentWindow?a=c.contentWindow.document.body.innerHTML:c.document&&(a=c.document.body.innerHTML);a&&"escape"==b.params.encoding&&(a=a.replace(/&quot;/g,'"').replace(/&gt;/g,">").replace(/&lt;/g,"<").replace(/&amp;/g,"&"));a=b.parseFeatures(a);b.replaceFeatures&&b.layer.destroyFeatures();a?(b.layer.addFeatures(a),b.layer.map.zoomToExtent(b.bounds),b.visibleOnUpload&&b.layer.setVisibility(!0),setTimeout(function(){b.deactivate()},250)):(b.deactivate(),
b.showMessage(OpenLayers.i18n("oleUploadFeatureNone")))};c.addEventListener&&c.addEventListener("load",d,!0);c.attachEvent&&c.attachEvent("onload",d);document.getElementById("upload_form").submit()}else this.cleanUp(this),this.showMessage(OpenLayers.i18n("oleUploadFeatureNoFile"),OpenLayers.Function.bind(this.delayedOpenDialog,this))},createInputElm:function(a,b,c,d,e){var f=document.createElement("input");f.setAttribute("type",c);f.id=a;f.name=b;f.value=d?d:null;e&&e.appendChild(f);return e},fileInputValue:function(a){return a.files?
1==a.files.length?a.files[0].name:null:a.value},setReplaceFeatures:function(a){this.replaceFeatures=a},CLASS_NAME:"OpenLayers.Editor.Control.UploadFeature"});/*
  2011 geOps
 @license    https://github.com/geops/ole/blob/master/license.txt
 @link       https://github.com/geops/ole
*/
...
b,c);if(!a._moving){var f=a.dragControl.handlers.drag.evt,k=!(!a._setfeature&&a.preserveAspectRatio)&&!(f&&f.shiftKey),g=new OpenLayers.Geometry.Point(d,e),f=a.center;this.rotate(-a.rotation,f);g.rotate(-a.rotation,f);var h=this.x-f.x,m=this.y-f.y,l=h-(this.x-g.x),n=m-(this.y-g.y);a.irregular&&!a._setfeature&&(h-=(this.x-g.x)/2,m-=(this.y-g.y)/2);this.x=d;this.y=e;g=1;a.feature.geometry instanceof OpenLayers.Geometry.Point?m=1:k?(m=1E-5>Math.abs(n)?1:m/n,g=(1E-5>Math.abs(l)?1:h/l)/m):(l=Math.sqrt(l*
l+n*n),m=Math.sqrt(h*h+m*m)/l);a._moving=!0;a.box.geometry.rotate(-a.rotation,f);delete a._moving;a.box.geometry.resize(m,f,g);a.box.geometry.rotate(a.rotation,f);a.transformFeature({scale:m,ratio:g});a.irregular&&!a._setfeature&&(h=f.clone(),h.x+=1E-5>Math.abs(d-f.x)?0:this.x-d,h.y+=1E-5>Math.abs(e-f.y)?0:this.y-e,a.box.geometry.move(this.x-d,this.y-e),a.transformFeature({center:h}))}},f=function(b,c){var d=this.x,e=this.y;OpenLayers.Geometry.Point.prototype.move.call(this,b,c);if(!a._moving){var f=
a.dragControl.handlers.drag.evt,f=f&&f.shiftKey?45:1,g=a.center,k=this.x-g.x,h=this.y-g.y;this.x=d;this.y=e;d=Math.atan2(h-c,k-b);d=Math.atan2(h,k)-d;d*=180/Math.PI;a._angle=(a._angle+d)%360;d=a.rotation%f;if(Math.abs(a._angle)>=f||0!==d)d=Math.round(a._angle/f)*f-d,a._angle=0,a.box.geometry.rotate(d,g),a.transformFeature({rotation:d})}},k=Array(8),g=Array(4),h,n,p,q="sw s se e ne n nw w".split(" "),l=0;8>l;++l)h=this.box.geometry.components[l],n=new OpenLayers.Feature.Vector(h.clone(),{role:q[l]+
"-resize"},"string"==typeof this.renderIntent?null:this.renderIntent),0==l%2&&(p=new OpenLayers.Feature.Vector(h.clone(),{role:q[l]+"-rotate"},"string"==typeof this.rotationHandleSymbolizer?null:this.rotationHandleSymbolizer),p.geometry.move=f,h._rotationHandle=p,g[l/2]=p),h.move=b,h.resize=c,h.rotate=d,n.geometry.move=e,h._handle=n,k[l]=n;this.rotationHandles=g;this.handles=k}});OpenLayers.Editor.Control.FixedAngleDrawing=OpenLayers.Class(OpenLayers.Control,{CLASS_NAME:"OpenLayers.Editor.Control.FixedAngleDrawing",active:!1,sketchVerticesAmount:null,guides:null,initialize:function(a){this.guides=[];OpenLayers.Control.prototype.initialize.call(this);this.layer=a},activate:function(){var a=OpenLayers.Control.prototype.activate.call(this);if(a)this.layer.events.on({sketchstarted:this.onSketchStarted,sketchmodified:this.onSketchModified,sketchcomplete:this.onSketchComplete,scope:this});
...
f.b,d),f=this.createWorldBoundaryPoint(f.m,d.right,f.b,d));d=a.geometry.components[0];d.x=c.x;d.y=c.y;var k=a.geometry.components[1];k.x=f.x;k.y=f.y;c=OpenLayers.Layer.Vector.prototype.drawFeature.apply(this,arguments);d.x=e.x1;d.y=e.y1;k.x=e.x2;k.y=e.y2}else console.warn("Failed to render a feature",a)}return c},moveTo:function(a,b,c){OpenLayers.Layer.Vector.prototype.moveTo.apply(this,arguments);this.features.forEach(function(a){this.drawFeature(a)},this)}});/*
  2011 geOps
 @license    https://github.com/geops/ole/blob/master/license.txt
 @link       https://github.com/geops/ole
*/
